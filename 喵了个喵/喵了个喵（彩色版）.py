...
class ColorMiao(MiaoMiao):
    ...
    def play(self):
        while True:
            ...
            欢迎游玩《喵了个喵》
            ====================
            1.[{self.l1[-1][0] if self.l1 else '    '}]\t 2.[{self.l2[-1][0] if self.l2 else '    '}]\t 3.[{self.l3[-1][0] if self.l3 else '    '}]
            4.[{self.l4[-1][0] if self.l4 else '    '}]\t 5.[{self.l5[-1][0] if self.l5 else '    '}]\t 6.[{self.l6[-1][0] if self.l6 else '    '}]
            7.[{self.l7[-1][0] if self.l7 else '    '}]\t 8.[{self.l8[-1][0] if self.l8 else '    '}]\t 9.[{self.l9[-1][0] if self.l9 else '    '}]
            ====================
